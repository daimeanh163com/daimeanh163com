...
#pca analysis
setwd("~/desktop/10/PCA")                   #设置工作目录
data=read.table("risk.txt",header=T,sep="\t",row.names=1)   #读取表格
...
#pca 2d plot
library(ggplot2)
group=c(rep("high riskscore group",157),rep("low riskcsore group",158))  #对照组和实验组的样品数目
